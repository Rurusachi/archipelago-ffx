...
public unsafe partial class ArchipelagoFFXModule {
    ...
    public static char* get_event_name(u32 event_id) => FhUtil.get_fptr<AtelGetEventName>(0x4796e0)(event_id);
    public static int atel_stack_pop(i32* param_1, AtelStack* atelStack) => FhUtil.get_fptr<AtelStackPop>(0x0046de90)(param_1, atelStack);
    ...
    private static FhMethodHandle<Call.MsCheckLeftWindow> _MsCheckLeftWindow;
    private static FhMethodHandle<Call.MsCheckUseCommand> _MsCheckUseCommand;
    private static FhMethodHandle<Call.TOBtlDrawStatusLimitGauge> _TOBtlDrawStatusLimitGauge;

    private static Call.MsGetChr _MsGetChr;
    private static Call.MsGetComData _MsGetComData;
    private static Call.MsGetCommandUse _MsGetCommandUse;
    private static Call.MsGetCommandMP _MsGetCommandMP;
    private static Call.MsGetRamChrMonster _MsGetRamChrMonster;
    private static Call.TODrawCrossBoxXYWHC2 _TODrawCrossBoxXYWHC2;
    ...
    private static FhMethodHandle<Call.MsApUp> _MsApUp;
    ...
    private static ChN_ReadSystemMGRP _ChN_ReadSystemMGRP;
    ...
    private static SndSepPlay _SndSepPlay;
    ...
    public void init_hooks() {
        ...
        _AtelEventSetUp = new FhMethodHandle<AtelEventSetUp>(this, game, h_AtelEventSetUp, offset: 0x00472e90);
        ...
        // getCurrentPartySlots
        _MsGetSavePartyMember = new FhMethodHandle<MsGetSavePartyMember>(this, game, h_MsGetSavePartyMember, offset: 0x003853b0);


        _MsBtlListGroup = FhUtil.get_fptr<MsBtlListGroup>(0x0039d230);
        _MsBattleExe = new FhMethodHandle<MsBattleExe>(this, game, h_MsBattleExe, offset: 0x003810f0);
        ...
        // Overdrive Experiment
        _MsCheckLeftWindow = new FhMethodHandle<Call.MsCheckLeftWindow>(this, game, h_MsCheckLeftWindow, offset: 0x0038f750);
        _MsCheckUseCommand = new FhMethodHandle<Call.MsCheckUseCommand>(this, game, h_MsCheckUseCommand, offset: 0x0038c750);
        _TOBtlDrawStatusLimitGauge = new FhMethodHandle<Call.TOBtlDrawStatusLimitGauge>(this, game, h_TOBtlDrawStatusLimitGauge, offset: 0x00496050);

        _MsGetChr = FhUtil.get_fptr<Call.MsGetChr>(0x00394030);
        _MsGetComData = FhUtil.get_fptr<Call.MsGetComData>(0x0039a4c0);
        _MsGetCommandUse = FhUtil.get_fptr<Call.MsGetCommandUse>(0x0039a5c0);
        _MsGetCommandMP = FhUtil.get_fptr<Call.MsGetCommandMP>(0x0038d030);
        _MsGetRamChrMonster = FhUtil.get_fptr<Call.MsGetRamChrMonster>(0x0039af00);
        _TODrawCrossBoxXYWHC2 = FhUtil.get_fptr<Call.TODrawCrossBoxXYWHC2>(0x004f4b20);

        _eiAbmParaGet = new FhMethodHandle<eiAbmParaGet>(this, game, h_eiAbmParaGet, offset: 0x00654860);
        _FUN_00a48910 = new FhMethodHandle<FUN_00a48910>(this, game, h_FUN_00a48910, offset: 0x00648910);

        _MsApUp = new FhMethodHandle<Call.MsApUp>(this, game, h_MsApUp, offset: 0x00398a10);
        ...
        _ChN_ReadSystemMGRP = FhUtil.get_fptr<ChN_ReadSystemMGRP>(0x004368f0);
        ...
        _SndSepPlay = FhUtil.get_fptr<SndSepPlay>(0x00486db0);
        _SndSepPlaySimple = FhUtil.get_fptr<SndSepPlaySimple>(0x00486de0);
    ...
    public static int h_Map_800F(AtelBasicWorker* work, i32* storage, AtelStack* atelStack) {
    ...
    public static bool h_openFile(nint _this, nint filename, bool readOnly, nint unknown_1, nint unknown_2, nint unknown_3) {
        ...
        FhLog.Debug($"{_this}, {x}, {readOnly}, {unknown_1}, {unknown_2}, {unknown_3}");
        var result = _openFile.orig_fptr(_this, filename, readOnly, unknown_1, unknown_2, unknown_3);
        FhLog.Debug($"{result}, {*(int*)_this}, {*(int*)((int)_this + 4)}");
        return result;
    }
    ...
    private static void h_AtelEventSetUp(u32 event_id) {
        _AtelEventSetUp.orig_fptr(event_id);

        string event_name = Marshal.PtrToStringAnsi((isize)get_event_name(event_id))!;
        FhLog.Debug($"atel_event_setup: {event_name}");
        if (event_name == "hiku2100") {
            FhLog.Debug($"atel_event_setup: Inject set_airship_destinations call");
            u8* code_ptr = Atel.controllers[0].worker(0)->code_ptr;
            set(code_ptr, 0x26D1, AtelInst.PUSHII.build(0x1));
            set(code_ptr, 0x26D4, AtelInst.CALLPOPA.build(0x1B8)); // Common.obtainBrotherhood(1) = set_airship_destinations

            set(code_ptr, 0x4028, AtelInst.JMP.build(0x13A));
        }
    }

    private static void h_Common_obtainTreasureInit(AtelBasicWorker* work, i32* storage, AtelStack* atelStack) {
        int treasure_id = atelStack->values_as_int[1];
        //ArchipelagoGUI.lastTreasure = treasure_id;
        FhLog.Debug($"obtain_treasure: {treasure_id}");
        ArchipelagoClient.sendTreasureLocation(treasure_id);

        _Common_obtainTreasureInit.orig_fptr(work, storage, atelStack);
    }

    private static void h_Common_obtainTreasureSilentlyInit(AtelBasicWorker* work, i32* storage, AtelStack* atelStack) {
        int treasure_id = atelStack->values_as_int[0];
        //ArchipelagoGUI.lastTreasure = treasure_id;
        FhLog.Debug($"obtain_treasure_silently: {treasure_id}");
        ArchipelagoClient.sendTreasureLocation(treasure_id);
        _Common_obtainTreasureSilentlyInit.orig_fptr(work, storage, atelStack);
    }

    private static void h_Common_obtainBrotherhood(AtelBasicWorker* work, i32* storage, AtelStack* atelStack) {
        FhLog.Debug($"obtain_brotherhood");
        if (atelStack->size > 0) {
            if (atelStack->values_as_int[0] == 1) {
                FhLog.Info($"obtain_brotherhood: Set Airship Destinations");
    ...
    private static i32 h_Common_grantCelestialUpgrade(AtelBasicWorker* work, i32* storage, AtelStack* atelStack) {
        int character = atelStack->values_as_int[0];
        int level = atelStack->values_as_int[1];
        FhLog.Debug($"grant_celestial_upgrade: character={(PlySaveId)character}, level={level}");

        return _Common_grantCelestialUpgrade.orig_fptr(work, storage, atelStack);
    }
    private static i32 h_Common_setPrimerCollected(AtelBasicWorker* work, i32* storage, AtelStack* atelStack) {
        int primer = atelStack->values_as_int[0];
        FhLog.Debug($"set_primer_collected: Al Bhed Primer {primer + 1}");
    ...
    private static void update_region_state(bool map_changed) {
        ...
        if (current_region != RegionEnum.None && region_states.TryGetValue(current_region, out ArchipelagoData.ArchipelagoRegion? current_state)) {
            ...
            if (map_changed) {
                current_state.room_id = save_data->last_room_id;
    ...
    private static void on_map_change() {
        ...
        //if (id_to_region.TryGetValue(save_data->current_room_id, out RegionEnum region)) {
        if (id_to_regions.Contains(save_data->current_room_id)) {
            ...
            if (!region_is_unlocked[region]) {
                ...
                if (current_region != region) {
                    ...
                    FhLog.Debug($"transition_to_map: Rerouted to map={current_state.room_id}, entrance={current_state.entrance}");
    ...
    /*
    private static void on_transition(AtelStack* atelStack) {
        ...
        ArchipelagoData.ArchipelagoRegion? current_state;
        // Update region state
        if (current_region != RegionEnum.None && region_states.TryGetValue(current_region, out current_state)) {
            current_state = region_states[current_region];
            current_state.Story_progress = save_data->story_progress;
            current_state.room_id = save_data->current_room_id;
            current_state.entrance = save_data->current_spawnpoint;
        }

        if (ArchipelagoData.id_to_region.TryGetValue(map, out RegionEnum region)) {
            if (!region_is_unlocked[region]) {
                // Reroute to current room
                FhLog.Debug($"{region} is locked!");
                atelStack->values_as_int[0] = save_data->current_room_id;
                atelStack->values_as_int[1] = save_data->current_spawnpoint;
            }
            else if (save_data->current_room_id == 382) {
                current_region = region;
                current_state = region_states[region];
                save_data->story_progress = current_state.Story_progress;
                atelStack->values_as_int[0] = current_state.room_id;
                atelStack->values_as_int[1] = current_state.entrance;
                map = atelStack->values_as_int[0];
                entrance = atelStack->values_as_int[1];
                FhLog.Debug($"transition_to_map: Rerouted to map={map}, entrance={entrance}");
            }
        }
        else {
            current_region = RegionEnum.None;
    ...
    private static i32 h_Common_transitionToMap(AtelBasicWorker* work, i32* storage, AtelStack* atelStack) {
        int map = atelStack->values_as_int[0];
        int entrance = atelStack->values_as_int[1];
        FhLog.Debug($"transition_to_map: map={map}, entrance={entrance}");

        //update_region_state(atelStack);

        return _Common_transitionToMap.orig_fptr(work, storage, atelStack);
    }
    private static i32 h_Common_warpToMap(AtelBasicWorker* work, i32* storage, AtelStack* atelStack) {
        int map = atelStack->values_as_int[0];
        int entrance = atelStack->values_as_int[1];
        FhLog.Debug($"warp_to_map: map={map}, entrance={entrance}");
    ...
    private static void h_SgEvent_showModularMenuInit(AtelBasicWorker* work, i32* storage, AtelStack* atelStack) {
        ...
        if (menu == 0x40800001 || menu == 0x40800002 || menu == 0x40800003 || menu == 0x40800004 || menu == 0x40800005) {
            FhLog.Debug($"Skipping menu: type={menuTypeString}, index={index} {(unknown1 == 0x40 ? "" : $", Unknown1={unknown1}")} {(unknown2 == 0x00 ? "" : $", Unknown2={unknown2}")}");
        ...
        if (menuType == 0x80) {
            FhLog.Info($"Unknown tutorial?");
        }
        FhLog.Info($"Opening menu: type={menuTypeString}, index={index} {(unknown1 == 0x40 ? "" : $", Unknown1={unknown1}")} {(unknown2 == 0x00 ? "" : $", Unknown2={unknown2}")}");
        _SgEvent_showModularMenuInit.orig_fptr(work, storage, atelStack);
    }

    /*
    private static i32 h_Common_playFieldVoiceLineInit(AtelBasicWorker* work, i32* storage, AtelStack* atelStack) {
        var voice_line = atelStack->values_as_int[0];
        FhLog.Debug($"play_field_voice_line_init: voice_line={voice_line}");

        return _Common_playFieldVoiceLineInit.orig_fptr(work, storage, atelStack);
    }
    private static i32 h_Common_playFieldVoiceLineExec(AtelBasicWorker* work, AtelStack* atelStack) {
        FhLog.Debug($"play_field_voice_line_exec: param_1={atelStack->values_as_int[0]}");

        return _Common_playFieldVoiceLineExec.orig_fptr(work, atelStack);
    }
    private static i32 h_Common_playFieldVoiceLineResultInt(AtelBasicWorker* work, i32* storage, AtelStack* atelStack) {
        var voice_line = atelStack->values_as_int[0];
        FhLog.Debug($"play_field_voice_line_result_int: param_1={voice_line}");

        return _Common_playFieldVoiceLineResultInt.orig_fptr(work, storage, atelStack);
    }
    private static i32 h_after_voiceline_init(AtelBasicWorker* work, i32* storage, AtelStack* atelStack) {
        var input = atelStack->values_as_int[0];
        FhLog.Debug($"after_voiceline_init: input={input}");

        return _FUN_0085fb60.orig_fptr(work, storage, atelStack);
    }
    private static bool h_after_voiceline_exec(i32* param_1, i32* param_2) {
        FhLog.Debug($"after_voiceline_exec: param_1={*param_2}");
    ...
    public static void play_voice_line(int voice_line) {
        ...
        FhLog.Debug($"stack_size = {stack.size}, voice_line = {stack.values_as_int[0]}");

        int param_1 = 0;
        int param_2 = 0;
        h_Common_playFieldVoiceLineInit((AtelBasicWorker*)&param_1, &param_2, &stack);
        FhLog.Debug($"play_voice_line: after play_field_voice_line_init");

        var result = h_Common_playFieldVoiceLineExec((AtelBasicWorker*)&param_1, &stack);
        FhLog.Debug($"play_voice_line: after exec, result={result}");

        h_Common_playFieldVoiceLineResultInt((AtelBasicWorker*)&param_1, &param_2, &stack);
        FhLog.Debug($"play_voice_line: after play_field_voice_line_init");
        FhLog.Debug($"play_field_voice_line_result_int: Stack={stack.size}");
        stack.push_int(5);

        h_after_voiceline_init((AtelBasicWorker*)&param_1, &param_2, &stack);
        FhLog.Debug($"play_voice_line: after after_voiceline_init");

        h_after_voiceline_exec(&param_1, &param_2);
        FhLog.Debug($"play_voice_line: after after_voiceline_exec");
    }
     */
    ...
    public static i32 h_Common_addPartyMember(AtelBasicWorker* work, i32* storage, AtelStack* atelStack) {
        ...
        FhLog.Debug($"add_party_member: character={(PlySaveId)character}");
    ...
    public static i32 h_Common_removePartyMember(AtelBasicWorker* work, i32* storage, AtelStack* atelStack) {
        ...
        FhLog.Debug($"remove_party_member: character={(PlySaveId)character}");
    ...
    public static i32 h_Common_removePartyMemberLongTerm(AtelBasicWorker* work, i32* storage, AtelStack* atelStack) {
        ...
        FhLog.Debug($"remove_party_member_long_term: character={(PlySaveId)character}");

        return _Common_removePartyMemberLongTerm.orig_fptr(work, storage, atelStack);
    }
    public static i32 h_Common_setWeaponInvisible(AtelBasicWorker* work, i32* storage, AtelStack* atelStack) {
        uint character = (byte)atelStack->values_as_int[0];
        int state = atelStack->values_as_int[1];
        FhLog.Debug($"character={(PlySaveId)character}, state={state}");
        if (state != 0 && character_is_unlocked.TryGetValue((PlySaveId)character, out bool is_unlocked) && is_unlocked) {
            atelStack->pop_int();
            atelStack->pop_int();
            return 0;
        }
    ...
    public static i32 h_Common_putPartyMemberInSlot(AtelBasicWorker* work, i32* storage, AtelStack* atelStack) {
        ...
        FhLog.Debug($"put_party_member_in_slot: slot={slot}, character={(character != 0xff ? (PlySaveId)character : "Empty")}");
    ...
    public static void h_MsGetSavePartyMember(u32* param_1, u32* param_2, u32* param_3) {
        FhLog.Debug($"get_current_party_slots");
    ...
    private static u32 h_give_item(u32 param_1, i32 param_2) {
        FhLog.Debug($"give_item: {param_2} x {param_1}");

        return _FUN_007905a0.orig_fptr(param_1, param_2);
    }
    private static void h_give_key_item(u32 param_1) {
        FhLog.Debug($"give_key_item: {param_1}");

        _FUN_0088e700.orig_fptr(param_1);
    }
    private static void h_take_gil(i32 param_1) {
        FhLog.Debug($"take_gil: {param_1}");

        _FUN_00785a60.orig_fptr(param_1);
    }
    private static i32 h_give_weapon(Equipment* param_1) {
        FhLog.Debug($"give_weapon: {(i32)param_1}");

        return _FUN_007ab930.orig_fptr(param_1);
    }
    private static byte* h_read_from_bin(i32 param_1, i16* param_2, i32 param_3) {
        FhLog.Debug($"get_from_bin: {param_1}, {(i32)param_2}, {param_3}");
        FhLog.Debug($"bin_pointers: {(u32)param_2}, {(u32)(*takara_pointer)}, {(u32)(*buki_get_pointer)}");
        if (param_2 == (i16*)*takara_pointer) {
            FhLog.Debug($"get_from_bin: takara.bin");
        } else if (param_2 == (i16*)*buki_get_pointer) {
            FhLog.Debug($"get_from_bin: buki_get.bin");
        }
        var result = _FUN_007ab890.orig_fptr(param_1, param_2, param_3);
        FhLog.Debug($"get_from_bin: result = {((u32)result).ToString("X")}");
        return result;
    }
    private static ushort h_get_weapon_name(Equipment* param_1) {
        FhLog.Debug($"get_weapon_name: {(i32)param_1}");

        return _FUN_007a0d10.orig_fptr(param_1);
    }
    private static void h_get_weapon_model(u16 name_id, u8 owner, i32 unknown, u16* model_id_pointer) {
        FhLog.Debug($"get_weapon_model: {name_id}, {owner}, {unknown}, {*model_id_pointer}, ");

        _FUN_007a0c70.orig_fptr(name_id, owner, unknown, model_id_pointer);
    }
    private static void h_obtain_treasure_cleanup(BtlRewardData* param_1, i32 param_2) {
        FhLog.Debug($"obtain_treasure_cleanup");
    ...
    public static void obtain_item(uint item_id, int amount) {
        ...
        switch (item_type) {
            ...
            case 0x5:
                // Equipment
                item_id &= 0xff;
                UnownedEquipment* weapon_data = (UnownedEquipment*)h_read_from_bin((i32)item_id, (i16*)(*buki_get_pointer), 0);
                //var data = get_from_bin((i32)item_id, (i16*)0x12000C00, 0);
                FhLog.Debug($"obtain_item: Weapon {(i32)weapon_data}");
            ...
            case 0xE:
                // Region Unlock
                item_id &= 0xff;
                FhLog.Debug($"Region: {(RegionEnum)item_id}");
                region_is_unlocked[(RegionEnum)item_id] = true;
                break;
            case 0xF:
                // Party Member
                var char_id = item_id & 0xFF;
                FhLog.Debug($"Character: {(PlySaveId)char_id}");
        ...
        if (item_type == 0xA000) {
            ...
            item_id &= 0xff;
            UnownedEquipment* weapon_data = (UnownedEquipment*)read_from_bin((i32)item_id, (i16*)(*buki_get_pointer), 0);
            //var data = get_from_bin((i32)item_id, (i16*)0x12000C00, 0);
            FhLog.Debug($"obtain_item: Weapon {(i32)weapon_data}");
            BtlRewardData rewardData = new BtlRewardData();
            rewardData.gear_count = 1;
    ...
    public static void set_airship_destinations() {
        FhLog.Debug("set_airship_destinations");
    ...
    private static void h_eiAbmParaGet() {
        // Replace normal calculation with custom Archipelago-based calculation (if option enabled)
        FhLog.Debug("Calculating stats");
        _eiAbmParaGet.orig_fptr(); // security_cookie doesn't matter?
    }

    private static void h_FUN_00a48910(uint chr_id, int node_idx) {
        // TODO: Send Archipelago location when node is unlocked (if option enabled)
        FhLog.Debug($"Unlock node {node_idx} for {(PlySaveId)chr_id}");
        _FUN_00a48910.orig_fptr(chr_id, node_idx);
    }

    private static nint h_MsApUp(uint chr_id, Chr* chr, uint base_ap_add, nint param_4) {
        FhLog.Debug($"AP gain: character={(PlySaveId)chr_id}, ap={base_ap_add}");
        uint new_ap = (uint)Math.Min(Math.BigMul(base_ap_add, ap_multiplier), 999_999_999);
    ...
    private static uint h_MsCheckLeftWindow(uint chr_id) {
        ...
        if (_fullOverdrive != 0) {
            chr->ovr_charge = chr->ovr_charge_max;
        }
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if (com_id != 0xff) {
                ...
                bVar1 = _MsGetCommandUse(chr_id, com_id);
                if (!bVar1) {
                    if (command->limit_cost == 0) {
                        uVar2 = uVar2 | 1;
                    }
                    else {
                        uVar2 = uVar2 | 2;
    ...
    private static uint h_MsCheckUseCommand(uint chr_id, Command* command, int param_3) {
        Chr* chr;
        uint uVar1;
        uint uVar2;
        ...
        chr = _MsGetChr(chr_id);
        if ( (command->limit_cost == 0 || ((command->limit_cost <= chr->ovr_charge && (*(ushort*)((int)chr + 0x616) & 0x400) == 0 ) || btl->debug_always_available_overdrive)
             ) && ((command->flags_misc & 0x20000) == 0 || chr->status_suffer_turns_left.silence == 0)) {
            uVar1 = _MsGetCommandMP(chr_id, (uint)command);
            uVar2 = _MsGetRamChrMonster(chr_id);
            if (uVar2 == 1 || btl->debug_no_mp_cost) {
                uVar1 = 0;
                param_3 = 0;
            }
    ...
    private static void h_TOBtlDrawStatusLimitGauge(int current_parts, float param_2, float param_3, float param_4, float param_5) {
        ...
        Command* hovered_command = _MsGetComData(cur_btl_window->command_list[cur_btl_window->sel_idx], (int*)0x0);
        if (hovered_command->limit_cost > 0) {
            short gauge_parts = FhUtil.get_at<short>(0x21d0aa4);
            Chr* chr = _MsGetChr(cur_btl_window->cur_chr_id);
            float parts_ratio = (float)gauge_parts / chr->ovr_charge_max;

            float gauge_start = _graphicUiRemapX2(1f) + param_2;
            float y = _graphicUiRemapY2(1f) + param_3;
            float h = param_5 - _graphicUiRemapY2(2f);
            if (chr->ovr_charge >= hovered_command->limit_cost) {
                ...
                float used_start = gauge_start + ((param_4 - _graphicUiRemapX2(2f)) * (chr->ovr_charge - hovered_command->limit_cost) * parts_ratio) / gauge_parts;
                ...
                float overflow_end = gauge_start + ((param_4 - _graphicUiRemapX2(2f)) * hovered_command->limit_cost * parts_ratio) / gauge_parts;
